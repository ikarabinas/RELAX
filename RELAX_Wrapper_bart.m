...
for FileNumber=RELAX_cfg.FilesToProcess(1,1:size(RELAX_cfg.FilesToProcess,2))
	...
	RELAXProcessingRoundThreeAllParticipants = table();
	RELAXProcessingExtremeRejectionsAllParticipants = table();
	RELAX_issues_to_check = table();
	RELAX_issues_to_check_2nd_run = table();
    ...
    % Save original channel info before channel removal
    EEG.allchan = EEG.chanlocs;
    
    % Use the noisy channels from config
    if isfield(RELAX_cfg, 'ManualNoisyChannels') && ~isempty(RELAX_cfg.ManualNoisyChannels)
        noisyOut.noisyChannels.all = find(ismember({EEG.chanlocs.labels}, RELAX_cfg.ManualNoisyChannels));
    else
        noisyOut.noisyChannels.all = [];  % No manual channels to remove
    end

    % Print channel names to be removed by Noisy Channels
    ...
    continuousEEG=EEG;
    EEG.RELAX.ListOfChannelsAfterRejections={EEG.chanlocs.labels}; % Get list of good channels for re-referencing
    ...
    %% Interpolate rejected electrodes prior to running GEDAI - IMK added
    ...
    if RELAX_cfg.Run_GEDAI==1
        [EEG, EEGartifacts, SENSAI_score, SENSAI_score_per_band, ...
            artifact_threshold_per_band, mean_ENOVA, ...
            ENOVA_per_epoch, ~, ENOVA_per_band] = ...
        GEDAI(EEG, 'auto', 12, 0.5, 'interpolated', true, false);
    end
