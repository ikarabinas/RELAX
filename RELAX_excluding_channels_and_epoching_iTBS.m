...
function [continuousEEG, epochedEEG] = RELAX_excluding_channels_and_epoching_iTBS(continuousEEG, RELAX_cfg)
    ...
    if exist('RELAX_cfg', 'var')==1
        ...
        if isfield(RELAX_cfg, 'ProportionOfExtremeNoiseAboveWhichToRejectChannel')==0
            RELAX_cfg.ProportionOfExtremeNoiseAboveWhichToRejectChannel=0.2; % If a channel shows extreme outlying data for more than this proportion of the total data, it is deleted
        end
    ...
    % Custom epoching bounded by TMS trains:
    epochedEEG = continuousEEG;
    
    TMS_idx = find(strcmp({epochedEEG.event.type}, 'TMS'));
    start_latencies = 1;
    end_latencies = [];
    
    for i = 1:length(TMS_idx)
        if epochedEEG.event(TMS_idx(i)).triplet == 1
            end_latencies(end+1) = epochedEEG.event(TMS_idx(i)).latency;
        elseif epochedEEG.event(TMS_idx(i)).triplet == 10
            start_latencies(end+1) = epochedEEG.event(TMS_idx(i)).latency;
        end
    end
    end_latencies(end+1) = epochedEEG.pnts;
    
    epoch_len_samples = round(1.0 * epochedEEG.srate); % 1s epochs
    step_samples = round(0.500 * epochedEEG.srate);    % 0.5s overlap step
    
    old_evt_count = length(epochedEEG.event);
    new_evt_idx = 1;

    for k = 1:length(start_latencies)
        % Create X event latencies strictly within the valid inter-train intervals
        latencies = start_latencies(k) : step_samples : (end_latencies(k) - epoch_len_samples);
        for j = 1:length(latencies)
            epochedEEG.event(old_evt_count+new_evt_idx).type = 'X';
            epochedEEG.event(old_evt_count+new_evt_idx).latency = latencies(j);
            new_evt_idx = new_evt_idx + 1;
        end
    end
    
    if new_evt_idx > 1
        epochedEEG = eeg_checkset(epochedEEG, 'eventconsistency');
    end
